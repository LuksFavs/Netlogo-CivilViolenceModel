...
to setup
  ...
  ifelse model [
    create-citizens ( Population * ( 1 - policePresence)) [
      set shape "circle"
      set color blue
      set heading 0
      set myc color
  ...
  [
    create-bs ( Population * ( 1 - policePresence) / 2) [
      set shape "circle"
      set color blue
      set heading 0
      set myc color
...
to move
  if any? neighbors with [  not any? turtles-here ] and jailed = False [ move-to one-of neighbors with [ not any? turtles-here ]]
  if patch-here = patch 0 0 and jailed = False [ move-to one-of patches with [ not any? turtles-here ] ]
end

to setPP
  set Grievance ( Hardship * ( 1 - Legitimacy ) )
  set PP ( 1 - exp( - 2.3026 * ( (sum [ count cops-here ] of turtles with [distance myself < vision ]) / ( sum [ count citizens-here with [ color = red ] ] of turtles with [ distance myself < vision ] + 1) )))
end

to Rebel?
  if jailed = False [
    let term random-float JailMax
    set NetRisk Risk * PP * ( term ^ alpha)
    let Q Grievance - NetRisk
    ifelse Q > T [
      set color red
    ][
    set color myc
...
520
citzens
count citizens with [ color = blue ]
17
1
11
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count citizens with [ jailed = True ]"
